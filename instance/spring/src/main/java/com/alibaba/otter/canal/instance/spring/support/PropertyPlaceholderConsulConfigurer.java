 ...
 * @version 1.0.0
 */
public class PropertyPlaceholderConsulConfigurer extends org.springframework.beans.factory.config.PropertyPlaceholderConfigurer implements ResourceLoaderAware, InitializingBean {
    ...
    public void setConsulHost(String consulHost) {
        this.consulHost = resolveSystemPropertyPlaceholders(consulHost);
    }

    public void setConsulToken(String consulToken) {
        this.consulToken = resolveSystemPropertyPlaceholders(consulToken);
    }

    public void setAppName(String appName) {
        this.appName = resolveSystemPropertyPlaceholders(appName);
    }

    public void setAppProfile(String appProfile) {
        this.appProfile = resolveSystemPropertyPlaceholders(appProfile);
    }
    ...
    public PropertyPlaceholderConsulConfigurer(){
        setIgnoreUnresolvablePlaceholders(true);
    }

    public void initConsulProperties() {
        ConsulClient consulClient = new ConsulClient(consulHost);
        String prefix = "config/" + appName + "::" + appProfile + "/canal/";

        Response<List<GetValue>> keyValueResponse = consulClient.getKVValues(prefix, consulToken);

        Properties consulProperties = new Properties();
        for (GetValue getValue : keyValueResponse.getValue()) {
            String confKey = "canal." + StringUtils.substringAfter(getValue.getKey(), prefix).replaceAll("/", ".");
            consulProperties.put(confKey, getValue.getDecodedValue());
        }
        this.setProperties(consulProperties);
    }
